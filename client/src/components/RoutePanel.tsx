...
export default function RoutePanel() {
  ...
  return (
    <div className="rp-root">
      <button className="rp-toggle" onClick={() => setOpen(o => !o)} title="Routes" aria-label="Toggle route panel" >☰</button>
      ...
      {open && (
        <div className="rp-panel">
          ...
          <div className="rp-tabs">
            {ROUTE_MODES.map(m => (
              <button key={m.id} className={`rp-tab ${activeMode === m.id ? 'active' : ''}`} onClick={() => handleModeChange(m.id)}>{m.label}</button>
            ))}
          </div>
          ...
          <div className="rp-search-wrap">
            ...
            <input className="rp-search" type="text" placeholder="Search routes or operator..." value={search} onChange={e => setSearch(e.target.value)} />
            {search && (
              ...
              return (
                <div key={agency} className="rp-group">
                  <div className="rp-group-header">
                    <button className="rp-group-toggle" onClick={() => toggleGroup(agency)} aria-expanded={expanded}>
                    ...
                    <div className="rp-group-actions">
                      <button className={`rp-btn-tiny ${allOn ? 'active' : ''}`} onClick={() => toggleAgency(activeMode, agency, true)}>on</button>
                      <button className={`rp-btn-tiny ${allOff ? 'active' : ''}`} onClick={() => toggleAgency(activeMode, agency, false)}>off</button>
                    </div>
                  </div>
                    ...
                    <div className="rp-group-items">
                      {agencyRoutes.map(r => (
                        <label key={r.route_id} className="rp-route">
                          <input type="checkbox" checked={!disabledRoutes.has(r.route_id)} onChange={() => toggleRoute(r.route_id)} />
