...
export default function SettingsPanel() {
  ...
  return (
    <div className="settings-root">
      <button className="settings-toggle" onClick={() => setOpen(o => !o)} title="Settings" aria-label="Toggle settings">⚙</button>
        ...
        <div className="settings-panel">
          ...
          <section>
            ...
            <div className="theme-grid">
              {Object.values(themes).map(theme => (
                <button key={theme.id} className={`theme-btn ${settings.themeId === theme.id ? 'active' : ''}`} onClick={() => update({ themeId: theme.id })} style={{ '--theme-road': theme.roadMotorway, '--theme-bg': theme.background } as React.CSSProperties}> <span className="theme-preview" />{theme.label}</button>
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <label className="toggle-row">
              ...
              <span className="dot" style={{ background: t.bus }} />Buses
            </label>
            <label className="toggle-row">
              <input type="checkbox" checked={settings.showTrains} onChange={e => update({ showTrains: e.target.checked })} />
              <span className="dot" style={{ background: t.train }} />Trains
            </label>
            <label className="toggle-row">
              <input type="checkbox" checked={settings.showIncidents} onChange={e => update({ showIncidents: e.target.checked })} />
              <span className="dot" style={{ background: t.incident }} />Incidents
            </label>
            <div className="interval-row">
              Icon size
              <div className="icon-size-controls">
                <input type="range" min={0.2} max={1.0} step={0.05} value={settings.vehicleIconSize} onChange={e => update({ vehicleIconSize: Number(e.target.value) })} className="icon-size-range" />
                <span className="icon-size-value">{settings.vehicleIconSize.toFixed(2)}</span>
              </div>
            </div>
          </section>
